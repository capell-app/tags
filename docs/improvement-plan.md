...
7. **Done/Shipped: populated screenshot media is promoted.** — The manifest now promotes the extension card, seeded tag index, create/edit form, and tag relation-manager captures, while `docs/screenshots.json` keeps optional host `TagsInput` evidence for consumers such as Blog. — `capell.json`, `docs/screenshots.json` — S
...
**Screenshot/media status.** Manifest promotes the extension card, populated tag index, create/edit form, and relation-manager captures. `docs/screenshots.json` also keeps a host `TagsInput` contract for Blog or another consumer form, with committed light/dark PNG evidence.
...
| Done/Shipped: Recapture populated index, relation-manager, and host TagsInput screenshots. Evidence: manifest promotes the seeded tag index, create/edit form, and relation-manager captures, and `docs/screenshots.json` keeps optional host TagsInput evidence. | Done   | S      | Med    | §2.7, §5    |
